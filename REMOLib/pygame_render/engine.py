...
class RenderEngine:
    ...
    def __init__(self, screen_width: int, screen_height: int,
        ...
        vertex_src = resources.read_text(
            'REMOLib.pygame_render', 'vertex.glsl')
        fragment_src_draw = resources.read_text(
            'REMOLib.pygame_render', 'fragment_draw.glsl')
        ...
        self._shader_draw["alpha"] = 1.0
        self._alpha_values = np.linspace(0, 1.0, 256) # 0-255 alpha값을 매핑하기 위한 배열
    ...
    def render(self,
        ...
        # If shader is None, use a default shader
        if shader is None:
            shader = self._shader_draw

        # Set alpha in the shader
        alpha = int(alpha)
        shader['alpha'] = self._alpha_values[alpha]  # alpha를 셰이더에 전달
    ...
    def rotozoom(self,
        ...
        tex = self.surface_to_texture(tex)
        ...
        dest_vertices = create_rotated_rect((0,0), tex.width,
                                            tex.height, scale, angle, flip)
        
        
        x,y,w,h = get_bounding_rectangle(dest_vertices)
        dest_vertices = [(v[0]-x,v[1]-y) for v in dest_vertices]


        
        temp_layer = self.make_layer((w,h))
        ...
        # Render the texture
        self.render_from_vertices(
            tex, temp_layer, dest_vertices, section_vertices,self._shader_draw)
        
        
        surface = self.texture_to_surface(temp_layer.texture)
        temp_layer.release()
        tex.release()
    ...
    def smoothscale(self,
        ...
        scale = (w/tex.width,h/tex.height)

        # Get the vertex coordinates of a rectangle that has been rotated,
        # scaled, and translated, in world coordinates
        
        dest_vertices = create_rotated_rect((0,0), tex.width,
                                            tex.height, scale, 0, (False,False))
    ...
    def copy(self,
        ...
        dest_vertices = create_rotated_rect((0,0), tex.width,
                                            tex.height, (scale,scale), angle, (False,False))
        
        temp_layer = self.make_layer((tex.width,tex.height))
        ...
        temp_layer._fbo.release()
        
        return temp_layer.texture
    
    def texture_to_surface(self,tex:Texture):
        data = tex.read()
        sfc = pygame.image.fromstring(data, (tex.width, tex.height), 'RGBA')
        return pygame.transform.flip(sfc, False, True)
