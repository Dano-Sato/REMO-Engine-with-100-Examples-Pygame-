...
class SurvivalScene(Scene):
    ...
    def initOnce(self) -> None:
        ...
        window_width, _ = Rs.getWindowRes()
        layout_left = 40
        ...
        sidebar_x = self.grid_panel.rect.right + 36
        ...
        card_panel_width = min(1320, max(880, window_width - layout_left * 2))
        self.card_panel = rectObj(
            pygame.Rect(0, 0, card_panel_width, 280), color=Cs.dark(Cs.slategray), edge=6, radius=30
        )
        self.card_panel.x = layout_left
        self.card_panel.y = 780

        self.card_layout = cardLayout(
            RPoint(self.card_panel.rect.x + 40, self.card_panel.rect.y + 40),
            spacing=40,
            maxWidth=self.card_panel.rect.width - 80,
            isVertical=False,
        )

        self.status_layout = layoutObj(pos=RPoint(sidebar_x, 60), spacing=16, isVertical=True)
        ...
        self.selection_panel.x = sidebar_x
        self.selection_panel.y = 400
        ...
        self.log_box = longTextObj(
            ...
            size=22,
            color=Cs.white,
            textWidth=self.log_panel_bg.rect.width - 48,
        )
        self.log_box.setParent(self.log_panel_bg, depth=1)

        self.end_day_button = monoTextButton("하루 마감", pos=(sidebar_x, 80), size=32, color=Cs.orange)
        self.end_day_button.connect(self._start_next_day)
        self.end_day_button.centerx = self.selection_panel.rect.centerx
